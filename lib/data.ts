//
// IMPORTANT!
// You MUST assume these functions are real, ie you may NOT modify them.
//

function getDataForWidth(width: number) {
  return [
    {
      key: "1",
...
export async function getFeaturedTravelDestinations(width: number) {
  await timeout(2_000);

  return getDataForWidth(width);
}

export async function GetTravelDestinationById(width: number, id: number) {
  await timeout(500);

  const data = getDataForWidth(width);

  const arrIndex = id - 1;

  if (arrIndex < 0 || data.length < arrIndex + 1) {
    throw new Error("Invalid id");
  }

  return data[arrIndex];
}
